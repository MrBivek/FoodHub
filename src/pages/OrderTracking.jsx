    ...
    <div className="min-h-screen bg-yellow-50 py-16 px-6">
      <div className="max-w-4xl mx-auto bg-white/70 backdrop-blur-md rounded-3xl shadow-2xl p-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between mb-10 gap-6">
          <p className="text-gray-600 text-lg">Order ID: <span className="font-semibold">{order.id}</span></p>
          <p className="text-gray-600 text-lg">Placed on: <span className="font-semibold">{order.createdAt.toLocaleString()}</span></p>
        </div>

        {/* Premium Progress Bar */}
        <div className="relative mb-12">
          <div className="absolute top-1/2 left-0 w-full h-1 -translate-y-1/2 bg-gray-200 rounded-full"></div>
          <div className="absolute top-1/2 left-0 h-1 -translate-y-1/2 bg-yellow-400 rounded-full"
        ...
        <div className="space-y-3">
          {order.items.map((item) => (
            <div key={item.id} className="flex justify-between items-center bg-white/50 backdrop-blur-md rounded-xl p-4 hover:shadow-lg transition shadow-md">
              <div className="flex items-center gap-4">
                <img src={item.img} alt={item.name} className="w-16 h-16 rounded-lg object-cover border border-yellow-200"/>
